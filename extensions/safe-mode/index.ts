...
export function styleMode(ctx: ExtensionContext, mode: SafeMode, outerAccess: boolean): string {
	// Show only a shield for yolo+ in the status bar; commands and state keep the yolo+ name.
	if (mode === "yolo" && outerAccess) return "\x1b[48;5;88;38;2;211;143;143m󰕥\x1b[0m";
...
export default function safeModeExtension(pi: ExtensionAPI): void {
	...
	const unsubscribeRenewRequest = pi.events.on("px:renew:settings:request", (payload) => {
		...
		pi.events.emit("px:renew:settings:response", {
			...
			cwd: request.cwd,
			state: { mode, outerAccess, sessionApprovedBashCommands: [...autoApprovedBashCommandsForSession] },
		});
	});
	...
	const unsubscribeRenewApply = pi.events.on("px:renew:settings:apply", (payload) => {
		...
		if (
			...
			request.owner !== "safe-mode" ||
			request.targetSessionId !== activeContext.sessionManager.getSessionId() ||
			request.cwd !== activeContext.cwd
		) return;
