...
import { MAX_PARALLEL_TASKS } from "./prepare.ts";
...
// Re-exported for callers/tests that imported the limit from the dispatch seam.
...
export interface DispatchTask {
	...
	task: string;
	cwd?: string;
	/** Pre-allocated run ID from preparation; generated by the child runner if omitted. */
	runId?: string;
}
...
export async function runDispatch(
	...
	if (request.tasks && request.tasks.length > 0) {
		// Backstop: preparation rejects oversized parallel requests before a
		// dispatch is accepted. Kept here until Stage 3 routes this runner off a
		// prepared dispatch (which can never exceed the limit).
