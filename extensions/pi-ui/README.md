...
- Lock mode shows `LOCKED ` (Nerd Font `fa-lock`) instead of the editor, without clearing the draft. Terminal keys (including Esc, Ctrl+C, other shortcuts, and mouse input) are ignored until you open Quick Actions with `Ctrl+,` and toggle Lock. While locked, only the Lock action is available in Quick Actions; unlocking closes the dialog automatically. Lock state resets on session shutdown. The locked editor display uses the status-bar extension. In `new` display mode, border stats remain visible and update while the editor body shows dim gray diagonal tape, muted gray rules, a bright `  LOCKED  ` label, and a muted unlock hint. The bands expand across most of the available width. Narrow terminals use a simpler lock motif.
