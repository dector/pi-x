...
- Lock mode shows `LOCKED ` (Nerd Font `fa-lock`) instead of the editor, without clearing the draft. Terminal keys (including Esc, Ctrl+C, other shortcuts, and mouse input) are ignored until you open Quick Actions with `Ctrl+,` and toggle Lock. While locked, only the Lock action is available in Quick Actions; unlocking closes the dialog automatically. Lock state resets on session shutdown. The locked editor display uses the neo-bar extension. In `new` display mode, border stats remain visible and update while the editor body shows dim gray diagonal tape, muted gray rules, a bright `  LOCKED  ` label, and a muted unlock hint. The bands expand across most of the available width. Narrow terminals use a simpler lock motif.
- The dialog uses the neo-bar frame language: white heavy lines (`━`/`┃`), rounded light corners, and tapered `╾`/`╼` joins where the border touches the title.
...
The selected entry is marked by the active **selection marker** (`PI_UI_SELECTION_MARKER`, or `/px:pi-ui-marker` at runtime). The default `chip` marker draws a right-aligned intense purple pill with pure white text, led by a chevron-down and closed by a fold affordance for entries that can collapse (`nf-md-unfold_more` when collapsed, `nf-md-unfold_less` when expanded), e.g. `󰅀 tool · 310/312 󰕏`: the entry label is italic, the background is a fixed branded purple (`#5b21b6`) on truecolor terminals and the theme's own purple on 256-colour terminals, it appears at the entry's own screen row, and it disappears after 1.5 s. The `none` marker draws nothing. Adding a marker is a single entry in `SELECTION_MARKERS` in `index.ts`.
