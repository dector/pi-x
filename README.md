...
| [`focus-mode`](extensions/focus-mode/README.md) | Keeps pi inside a reading column on wide monitors: `/px:focus [on [N]\|off\|set N[/bias]\|bias [N]\|config\|status]`, persisted globally, default 100 columns centered, `bias -100` moves it flush left, no margin on a screen that already fits. `/px:focus config` is a settings dialog with a live preview, presets, and Apply / Apply-for-session. | |
