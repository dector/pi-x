...
| [`subagent`](extensions/subagent/README.md) | Delegates single, parallel, and chained tasks to isolated RPC child agents with permission inheritance, approval relay, `/px:agents` runtime controls, a running-count status row, and `/px:agent:log`. | `status-bar`, `safe-mode` optional |
