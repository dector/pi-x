...
| [`reset`](extensions/reset/README.md) | Adds `/reset` for a fresh unnamed session that retains supported settings; `/reset -proc` stops managed processes. `+agents` is refused. | `safe-mode`, `permissions-core`, `review-level` optional; `proc` for `-proc` |
